...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(projectForm) {
        projectForm.addEventListener('submit', async (e) => {
            ...
            const client = document.getElementById('proj_client').value;
            ...
            submitBtn.innerText = "Hashing File...";
            ...
            try {
                ...
                if (!hashRes.ok || !hashData.hash) {
                    throw new Error(hashData.error || "Hashing failed");
                }

                // 2. Sign via Frontend (MetaMask)
                submitBtn.innerText = "Confirm in Wallet...";
                
                // This calls the function from credchain.js
                const receipt = await addProjectOnChain(
                    client, 
                    name, 
                    desc, 
                    lang, 
    ...
    try {
        ...
        if (profileRes.ok) {
            ...
            if (pdata.skills && pdata.skills.length > 0) {
                skillsEl.innerHTML = pdata.skills.map(s => `<span class="bg-primary-dark text-blue-300 text-xs font-semibold px-2 py-1 rounded border border-gray-600">${s}</span>`).join('');
            } else {
